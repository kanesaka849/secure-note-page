...
###SCHEDULE_SECTION###
  ...
  <div class="card full">
    ...
    <div style="margin-top:10px;padding:10px 12px;border:2px solid #e05555;border-radius:8px;font-size:12px;">
      <strong>🎤 面接当日の流れ（10/19）</strong>：✅確実（公式）＝個別<strong>30分</strong>・<strong>自己推薦書中心</strong>＋学習適応性の評価／△推定（他学群体験記2件・体専は未確認）＝面接官<strong>3名</strong>（AC教員1＝進行＋学類教授2）<br>
      ①雑談（緊張ほぐし）→ ②<strong>「書類提出後に研究が進んでいれば説明して」</strong>→ ③<strong>「自己推薦書を5分で要約して」</strong>→ ④研究の深掘り（学会質疑スタイル）→ ⑤なぜAC入試か・志望理由 → ⑥大学での計画・将来（※①〜⑥は体験記由来の推定。順番が違っても材料は「書類＋志望＋計画」で確定なので、要約は<strong>5分版・2分版・30秒版</strong>を用意しどう振られても対応）<br>
      ⚠️ 待合室は電子機器不可 → <strong>紙の書類一式（FINAL 30p控え）持参</strong><br>
      <strong>分析ポイント</strong>：面接の正体＝「人物が書類に見合うか」の検証（生成AI念頭・大学側発言）。評価＝コミュニケーション＋<strong>インテグリティ（誠実さ）</strong>→ 正直な未達の書類は面接自体が得点機会。体専想定質問＝「科学的根拠は？」（Weyand・木越論文）／「失敗したらどう修正？」（限界→筑波で研究デザイン＝書類の主軸のまま）／「誰かに手伝った？」（支援開示と同内容を口頭で）。暗記でなく対話・楽しむ・素直さ。詳細＝output/面接対策_外部情報調査_2026-09.md
    </div>
  </div>
